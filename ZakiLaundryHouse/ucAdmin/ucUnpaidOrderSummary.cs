...
{
    public partial class ucUnpaidOrderSummary : UserControl
    {
        ...
        private void ucUnpaidOrderSummary_Load(object sender, EventArgs e)
        {
            // ===== HEADER =====
            ...
            // ===== ITEMS =====
            LoadOrderItems(); // Make sure this method is also SQLite-compatible
        }
        private void LoadOrderItems()
        {
            ...
            using (var conn = new SQLiteConnection(connStr))
            {
                ...
                // ===== Fetch order remarks =====
                using (var cmdRemarks = new SQLiteCommand(
...
LIMIT 1"; // SQLite
                                ...
                                using (var cmd2 = new SQLiteCommand(priceQuery, conn2))
                                {
                                    cmd2.Parameters.AddWithValue("@ItemID", itemId);
                                    using (var priceReader = cmd2.ExecuteReader())
                                    {
                                        if (priceReader.Read())
                                        {
                                            price = priceReader["Price"] != DBNull.Value ? Convert.ToDecimal(priceReader["Price"]) : 0;
                            ...
                            switch (pricingType)
                            {
                                case "Per KG": total = price * weight * qty; break;
                                case "Per Load":
                                case "Per Minimum":
                                case "Fixed":
                                default: total = price * qty; break;
                            }
                            amount = total;
                            ...
                            string itemLine;
                            if (hideWeight)
                                itemLine = string.Format("{0,-13}{1,5}{2,10:0.00}{3,10:0.00}", nameLines[0], qty, price, amount);
                            else
                            {
                                string weightStr = weight > 0 ? weight.ToString("0.00") + "kg" : "";
                                itemLine = string.Format("{0,-13}{1,5}{2,10}{3,9:0.00}{4,10:0.00}", nameLines[0], qty, weightStr, price, amount);
                            }
                        ...
                        lblLineBottom.Location = new Point(10, y);
        ...
        private void BuildDrawItems()
        {
            ...
            // Sort draw items by Y ascending to ensure top-to-bottom order
            drawItems = drawItems.OrderBy(di => di.Y).ToList();
        }
        public void PrintReceipt()
        {
            ...
            // ✅ Get cashier name based on most recent login in LogTrail (SQLite version)
            string cashierName = "";
            string connStr = DbConnection.ConnectionString;
        ...
        WHERE L.ActionType = 'Login'
        ORDER BY L.DateTime DESC
        LIMIT 1;"; // SQLite uses LIMIT 1 instead of TOP 1
            ...
            lblCashier.Text = "Cashier: " + (string.IsNullOrEmpty(cashierName) ? "N/A" : cashierName);
            lblDateTime.Text = "Date: " + DateTime.Now.ToString("MMM dd, yyyy hh:mm tt");
            lblCustomerName.Text = "Customer: " + (string.IsNullOrEmpty(CustomerName) ? "N/A" : CustomerName);
            lblCustomerContact.Text = "Contact: " + (string.IsNullOrEmpty(CustomerContact) ? "N/A" : CustomerContact);
            ...
            // ✅ Important: Use StandardPrintController (no “Printing…” popup)
            printDocument.PrintController = new StandardPrintController();

            // ✅ Force actual printer (avoid default generic driver)
            printDocument.PrinterSettings = new PrinterSettings();
            printDocument.PrinterSettings.PrinterName = "XP-58"; // change to your actual printer name if needed

            // ✅ Paper size (for 58mm roll)
